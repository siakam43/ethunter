...
**修复**: `param_binding.analyze()` 接收 `symbol_table` 参数，使用 `symbol_table.all_function_names` 作为 `symbol_names`。`func_params` 仅用于参数名映射（`param_names = func_params.get(call_name, [])`），不用于符号名检查。
...
`VariableState.registered_callbacks` 在 `param_assign` callback_reg 产边时被写入，但搜索全部 analyzer 模块发现：**没有任何模块读取它**。可以安全删除。
...
Phase 1 重构中 `func_fp_params` 和 `param_usage` 因 `hasattr` 回退链问题留在 `dataflow.state` 上。旧代码移除后，这两个字段提升到 `DataflowEngine` 直接字段（已在 Task 1 中声明，现在可以安全写入）。

`param_helpers.prepare()` 改为直接写 `dataflow.func_fp_params.update(...)` 和 `dataflow.param_usage.update(...)`，替代当前 `dataflow.state.func_fp_params`。
...
| `src/ethunter/analyzer/orchestrator.py` | 移除 param_assign 引用 + Fix B 后处理 | 152 → 130 |
| `src/ethunter/analyzer/param_binding.py` | 接收 symbol_table + 三层 gating | 207 → 230 |
| `src/ethunter/analyzer/dataflow.py` | 删除 registered_callbacks + register_callback + 更新字段 | 222 → 210 |
| `src/ethunter/analyzer/param_helpers.py` | prepare() 写 engine 字段（不再写 state） | ~210 → ~210 |
| `src/ethunter/analyzer/field_call.py` | 移除旧格式双读 | 282 → 240 |
| `src/ethunter/analyzer/param_dispatch.py` | 读 engine 字段（func_fp_params 从 engine） | ~140 → ~140 |
| `src/ethunter/analyzer/callback_reg.py` | 读 engine 字段 | ~55 → ~55 |
