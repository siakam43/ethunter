"""CG-Bench benchmark: evaluate ethunter's indirect call detection recall.

Runs ethunter on each CG-Bench example fixture and compares detected
indirect edges against per-example ground_truth.json. Reports recall
per category and overall (measurement only, no pass/fail threshold).
"""
...
CG_BENCH_DIR = os.path.join(os.path.dirname(__file__), 'benchmark', 'cg_bench')
...
def _get_categories():
    ...
    if not os.path.isdir(CG_BENCH_DIR):
        return []
    return sorted(
        d for d in os.listdir(CG_BENCH_DIR)
        if os.path.isdir(os.path.join(CG_BENCH_DIR, d))
    )


def _get_examples(category):
    """Return list of example directory names for a category."""
    cat_dir = os.path.join(CG_BENCH_DIR, category)
...
def test_cg_bench_report():
    """Run ethunter on all CG-Bench fixtures and report recall per category and overall."""
    categories = _get_categories()
    assert categories, f'No category directories found in {CG_BENCH_DIR}'
    ...
    for category in categories:
        ...
        for example in examples:
            example_dir = os.path.join(CG_BENCH_DIR, category, example)
